class BallPlayer:
    def __init__(self, name: str, number: int, goals: int, assists: int, minutes: int):
        self.name = name
        self.number = number
        self.goals = goals
        self.assists = assists
        self.minutes = minutes

    def __str__(self):
        return f"BallPlayer(name={self.name}, number={self.number}, goals={self.goals}, passes={self.assists}, minutes={self.minutes})"


def most_goals(players: list) -> str:
    top_scorer = max(players, key=lambda p: p.goals)
    return top_scorer.name


def most_points(players: list) -> tuple:
    # en buyuk nesneyi goller + asistler degerlerine gore buluyoruz
    top_pointer = max(players, key=lambda p: p.goals + p.assists)
    return (top_pointer.name, top_pointer.number)


def least_minutes(players: list):
    return min(players, key=lambda p: p.minutes)


# Test Alanı
if __name__ == "__main__":
    player1 = BallPlayer("Archie Bonkers", 13, 5, 12, 46)
    player2 = BallPlayer("Speedy Tickets", 7, 2, 26, 55)
    player3 = BallPlayer("Cruella De Hill", 9, 1, 32, 26)
    player4 = BallPlayer("Devilled Tasmanian", 12, 1, 11, 41)
    player5 = BallPlayer("Donald Quack", 4, 3, 9, 12)

    team = [player1, player2, player3, player4, player5]
    print(most_goals(team))
    print(most_points(team))
    print(least_minutes(team))

    # Çıktı:
    #
    # Archie Bonkers
    # ('Cruella De Hill', 9)
    # BallPlayer(name=Donald Quack, number=4, goals=3, passes=9, minutes=12)
